...
import { ShieldAlert, AlertCircle, WifiOff, TrendingUp, ChevronRight } from "lucide-react";
...
function KpiCard({ icon: Icon, label, value, status, subText, onClick }) {
  return (
    ...
    >
      ...
      {subText && <span className="kpi-card-v2__sub">{subText}</span>}
      {onClick && <ChevronRight size={16} className="kpi-card-v2__arrow" />}
    </div>
  );
}
